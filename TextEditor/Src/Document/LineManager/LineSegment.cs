...
//									--j;
								...
								case '@': // matches @
									...
									break;
								case '^':
									//行頭に指定記号が無いときはfalseを返す
									++i;
									if (i < expr.Length)
									{
										char c = expr[i];
										if (c == document.GetCharAt(this.Offset + index + j))
										{
											if ((index + j) == 0)
											{
												//行頭に指定記号があるときはOKにする
											...
											for (int ti = 0; ti < (index + j); ti++)	//行頭から指定文字までをループ
											{
												char tc = document.GetCharAt(this.Offset + ti);
												if (tc != '\t')
												{
													//指定記号の前にタブ以外の物が存在するときはNGにする
													return false;
												}
											}
										}
