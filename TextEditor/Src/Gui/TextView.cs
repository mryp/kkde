...
//			bgColorBrush.Dispose();
		...
		int PaintLinePart(Graphics g, int lineNumber, int startColumn, int endColumn, Rectangle lineRectangle, int physicalXPos)
		{
			...
			repeatDrawCurrentWord:
				...
				else if (currentWordType == TextWordType.WideSpace)
					wordForeColor = spaceMarkerColor.Color;
				...
				else if (currentWord.Type == TextWordType.WideSpace)
				{
					++physicalColumn;

					wordRectangle = new RectangleF(physicalXPos, lineRectangle.Y, SpaceWidth * 2, lineRectangle.Height);
					g.FillRectangle(wordBackBrush, wordRectangle);
					
					if (TextEditorProperties.ShowWideSpaces) {
						DrawWideSpaceMarker(g, wordForeColor, physicalXPos, lineRectangle.Y);
					}
					physicalXPos += SpaceWidth*2;
				}
			...
			for (int i = 0; i < wordCount; i++) {
				...
				switch (word.Type) {
					case TextWordType.Space:
						...
						break;
					case TextWordType.WideSpace:
						drawingPos += spaceWidth * 2;
						break;
		...
		void DrawSpaceMarker(Graphics g, Color color, int x, int y)
		{
			...
			int startX = x + 1;
			int endX = startX + wideSpaceWidth - 2;
			int startY = y + fontHeight - 1;
			g.DrawLine(BrushRegistry.GetPen(spaceMarkerColor.Color), startX, startY, endX, startY);
			g.DrawLine(BrushRegistry.GetPen(spaceMarkerColor.Color), startX, startY, startX, startY - 1);
			g.DrawLine(BrushRegistry.GetPen(spaceMarkerColor.Color), endX, startY, endX, startY - 1);
			//DrawString(g, "\u2423", spaceMarkerColor.GetFont(TextEditorProperties.FontContainer), color, x, y);
		}

		void DrawWideSpaceMarker(Graphics g, Color color, int x, int y)
		{
			//全角スペース記号
			HighlightColor spaceMarkerColor = textArea.Document.HighlightingStrategy.GetColorFor("SpaceMarkers");
			DrawString(g, "\u25A1", spaceMarkerColor.GetFont(TextEditorProperties.FontContainer), color, x, y);
		...
		int DrawEOLMarker(Graphics g, Color color, Brush backBrush, int x, int y)
		{
			...
			int allowX = x + 1;
			int allowWidth = (int)((double)wideSpaceWidth - 1);
			int allowY = y + (int)Math.Round((double)fontHeight / 2.0);
			g.DrawLine(BrushRegistry.GetPen(eolMarkerColor.Color), allowX, allowY, allowX + allowWidth, allowY);
			g.DrawLine(BrushRegistry.GetPen(eolMarkerColor.Color), allowX + allowWidth, allowY, allowX + allowWidth, y + 2);

			//矢印の斜め部分を作成する
			int end = (int)((double)allowWidth * 0.5);
			for (int i = 0; i < end; i++)
			{
				g.DrawLine(BrushRegistry.GetPen(eolMarkerColor.Color), allowX + i, allowY + i, allowX + i + 1, allowY + i + 1);
				g.DrawLine(BrushRegistry.GetPen(eolMarkerColor.Color), allowX + i, allowY - i, allowX + i + 1, allowY - i - 1);
			}

			//没
			//g.DrawLine(BrushRegistry.GetPen(eolMarkerColor.Color), allowX, allowY, allowX + (int)((double)allowWidth * 0.5), y + (int)Math.Round((double)fontHeight * 0.7));
			//g.DrawLine(BrushRegistry.GetPen(eolMarkerColor.Color), allowX, allowY, allowX + (int)((double)allowWidth * 0.5), y + (int)Math.Round((double)fontHeight * 0.3));
